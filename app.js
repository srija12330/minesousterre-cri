...
// L'accès est déjà protégé par la connexion au portail (mine.sous-terre.cri).
// Ici, on identifie simplement la personne pour le suivi des saisies (qui a fait quoi).
...
function identifierUtilisateur() {
  let nom = localStorage.getItem('epi_nom_utilisateur');
  if (!nom) {
    nom = prompt('Quel est votre nom ? (pour identifier vos saisies)');
    if (nom && nom.trim()) {
      localStorage.setItem('epi_nom_utilisateur', nom.trim());
      nom = nom.trim();
    } else {
      nom = 'Inconnu';
    }
